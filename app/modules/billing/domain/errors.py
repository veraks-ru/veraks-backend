...
class InviteError(BillingError):
    """Базовая ошибка пригласительных ссылок."""


class InviteNotFoundError(InviteError):
    """Кода нет: опечатка в ссылке либо приглашение удалено."""


class InviteAlreadyRedeemedError(InviteError):
    """Ссылка одноразовая и уже использована другим человеком."""


class InviteRevokedError(InviteError):
    """Приглашение отозвано выдавшим его."""


class InvalidInviteError(InviteError):
    """Некорректные параметры приглашения (например, неположительный срок)."""
