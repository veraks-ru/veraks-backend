...
class User:
    ...
    * **email + одноразовая ссылка** — заполнен только ``email``, хэшей нет,
    ...
    def anonymize_for_deletion(self) -> bool:
        ...
        Стирает ФИО (``real_name_enc``), **обнуляет ``email``** и освобождает
        публичный псевдоним (``username`` → детерминированное «надгробие» вида
        ``deleted-<8 hex от id>``, ``display_name`` → «Удалённый аккаунт»).

        Почему email стирается, хотя это позволяет зарегистрироваться заново на
        тот же адрес: минимизация ПДн важнее антиобхода. Гарантию «один человек
        = один аккаунт» без государственной идентификации мы и так не даём
        (``identity_verified=False`` у всех, кто вошёл по ссылке на почту),
        поэтому удерживать адрес удалённого пользователя ради блокировки
        повторной регистрации юридически не оправдано — это хранение ПДн без
        основания. Когда ЕСИА вернут, гарантию восстановит ``snils_hash``.

        ``snils_hash``/``esia_oid_hash`` НЕ трогаем — это ключ того самого
        инварианта: без них повторная регистрация того же ГРАЖДАНИНА после
        удаления обошла бы ограничение. Правомерность и срок хранения хэшей
        после удаления — вопрос к юристу (см. ``audit/04-human-playbooks.md``
        §3 п.7); пока решение по умолчанию — хранить бессрочно, т.к. хэш
        необратим и сам по себе не раскрывает ПДн (в отличие от адреса).
