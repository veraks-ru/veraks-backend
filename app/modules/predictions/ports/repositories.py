...
class PredictionRepository(Protocol):
    ...
    async def list_for_user(
        ...
        ``resolved_only=True`` оставляет только засчитанные (есть
        ``brier_score``) и НЕ по аннулированным событиям — это публичный
        трек-рекорд, а аннулированное событие не участвует нигде.
        ``False`` — все, включая ожидающие разрешения и аннулированные (свой
        список, где рядом виден статус события).
        """
        ...
