...
def entry_payload(
    ...
    **Ограничение на типы значений внутри ``before``/``after``/``metadata``**:
    только JSON-нативные детерминированные типы — ``str``/``int``/``bool``/
    ``None`` и вложенные ``dict``/``list`` из них (как уже сделано здесь для
    ``actor_id``/``entity_id``/``occurred_at`` — они явно приводятся к строке
    ДО попадания в payload). Не кладите ``float``/``Decimal``: на записи они
    сериализуются нормально, но после круглого пути через ``jsonb`` Postgres
    иногда меняет численное представление (убирает незначащие нули, иначе
    форматирует крупные/мелкие числа) — при верификации значение,
    прочитанное обратно из БД, даст другую ``canonical_json`` и, значит,
    другой хеш, хотя по сути ничего не менялось. Это ложное срабатывание
    verify, а не порча цепочки. Если число важно для payload'а — кладите его
    строкой (как поступают с UUID/datetime).
    """
    return {
